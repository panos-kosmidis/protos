...
Driver.prototype.provideTo = function(context) {
  ...
  _.extend(context, Driver.prototype.__modelMethods);
...
Driver.prototype.cacheClientMethods = function(client) {
  ...
  var idx = client ? 1 : 0,
      methods = slice.call(arguments, idx),
  ...
  if (invalidate != null) {
    ...
    self.storage.delete(invalidate, function(err) {
      ...
      if (err) {
        ...
        app.debug(util.format("Invalidated cacheID '%s'", invalidateCacheIDs.join(', ')));
        ...
        if (cache != null) {
          ...
          params.push(function() {
            var cacheKey, queryResults,
                queryResults = slice.call(arguments, 0),
