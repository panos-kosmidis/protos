...
function CoreJS(noserver) {
  ...
  this.httpMethods = ['GET', 'POST', 'PUT', 'DELETE', 'OPTIONS', 'TRACE'];
  ...
  var Utility = require('./utility'),
      skipFromLibDir = ['request.js', 'response.js', 'framework.js', 'addons', 'drivers', 'engines', 'storages'];
  ...
  // Storage
  this.util.getFiles(this.path + '/lib/storages/' , function(file) {
    var key = file.replace(self.regex.jsFile, '');
    self.storages[key] = require(self.path + '/lib/storages/' + file);
  });
  
  // Database drivers
  this.util.getFiles(this.path + '/lib/drivers', function(file) {
    var key = file.replace(self.regex.jsFile, '');
    self.drivers[key] = require(self.path + '/lib/drivers/' + file);
  });
  ...
  // Template Engines
  this.util.getFiles(this.path + '/lib/engines', function(file) {
    var key = file.replace(self.regex.jsFile, '');
    self.engines[key] = require(self.path + '/lib/engines/' + file);
  });
...
/**
  ...
  to the framework instance on construction. This means, the methods and properties
...
CoreJS.bootstrap = function(dir, config) {
  ...
  var appEnv, envFunc, vhosts = {}, coreJS;
  if (process.env.NODE_ENV !== 'MASTER') {
    ...
    // Configure vhosts
    vhosts[config.host || 'localhost'] = dir;
    CoreJS.configure('vhosts', vhosts);
    ...
    // Framework singleton (automatically available via `framework` global)
    return new CoreJS();
  } else {
    // TODO: Master server
    console.exit('Master Server');
  }
}
...
CoreJS.prototype.addVhosts = function(servers) {
  var Application, app, appInit, data, host, initFunction, path, controllerProto,
      ...
      'storage', 'lib','models', 'helpers', 'controllers', 'engines', 'session'];
  
  for (host in servers) {
    path = pathModule.resolve(this.path, servers[host]);
...
function autoCurlRequest() {
  if (process.argv.length >= 3 && framework.config.autoCurl) {
