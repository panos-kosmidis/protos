...
Utility.prototype.typecast = function(value) {
  if (corejs.regex.integer.test(value)) {
    return parseInt(value, 10);
  } else if (corejs.regex.float.test(value)) {
    return parseFloat(value);
  } else if (corejs.regex["null"].test(value)) {
    return null;
  } else if (corejs.regex.boolean.test(value)) {
...
Utility.prototype.requireAllTo = function(path, destination, filterCb) {
  ...
  for (var key,file,i=0; i < files.length; i++) {
    ...
    file = file.replace(corejs.regex.jsFile, '');
