...
Session.prototype.create = function(req, res, data, persistent, callback) {
  ...
  multi.exec(function(err, replies) {
    if (err) app.log(err);
      ...
      self.app.emit('session_load', hashes.sessId, req.__session);
...
Session.prototype.loadSession = function(req, res, callback) {
  ...
  if (sessId) {
    this.storage.getHash(sessId, function(err, data) {
      ...
      if (err) {
        ...
        if (guest) {
          ...
          req.__origSessionState = _.extend({}, data);
          req.__sessionJson = JSON.stringify(data);
          self.app.emit('session_load', sessId, req.__session);
          callback.call(self);
        } else if (sessHash) {
    ...
    req.__sessionJson = '';
    this.app.emit('session_load', sessId, req.__session);
    callback.call(self);
  }
}
...
IncomingMessage.prototype.saveSessionState = function(callback) {
  ...
  multi.exec(function(err, replies) {
    if (err) {
      app.serverError(self.__response, util.format("Unable to save session state: %s" + err.toString()));
    } else {
...
IncomingMessage.prototype.sessionChanged = function() {
  if (!this.__app.supports.session) return false;
  var curSessionJson = JSON.stringify(this.__session);
  return (this.hasCookie(this.__app.session.config.sessCookie) 
    && curSessionJson !== this.__sessionJson 
    && curSessionJson !== '{}');
}
