...
/**
  ...
  If the class is not available in lib/, then the class will be created 
  from framework/lib.
...
Application.prototype.buildPartialView = function(path) {
  var pos = path.indexOf('.'),
      ...
      id = func.id = this.relPath(path.substr(0,pos), this.mvcpath + 'views')
...
return " + func.name + ";\n\n\
  ...
  newFunc = compile({
    ...
    __filename: this.mvcpath + 'controllers/' + func.name + '.js',
    process: process
  });
