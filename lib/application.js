...
function Application(domain, path) {
  ...
  framework.util.requireAllTo(this.path + "/app/helpers", this.helpers, function(Ctor) {
  ...
  framework.util.requireAllTo(this.path + "/app/models", this.models, function(Ctor) {
  ...
  // Get /app/controllers/
  var cpath = path + '/app/controllers/',
  ...
  this.views.static = framework.util.ls(this.path + "/app/views/" + this.paths.static, this.regex.templateFile);
  ...
  var partialPaths = [];
  fileModule.walkSync(this.path + '/app/views', function(dirPath, dirs, files) {
    var layoutPath = self.fullPath('/app/views/' + self.paths.layout);
...
Application.prototype.buildPartialView = function(path) {
  var pos = path.indexOf('.'),
      ...
      id = func.id = this.relPath(path.substr(0,pos), '/app/views')
...
return " + func.name + ";\n\n\
  ...
  newFunc = compile({
    ...
    require: require,
    console: console,
    __dirname: this.path + '/app/controllers',
    __filename: this.path + '/app/controllers/' + func.name + '.js',
    process: process
  });
