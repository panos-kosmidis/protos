...
/**
  ...
  @param {string} property  Property to attach the middleware into the app singleton
  @returns {object} instance of the component's function
  @public
 */

Application.prototype.use = function(middleware, options, property) {
  var Ctor, path = this.path + '/middleware/' + middleware + '.js';
  
  if ( pathModule.existsSync(path) ) {
    // Load application middleware
    Ctor = require(path);
  } else {
    // Load corejs middleware
    path = corejs.path + '/middleware/' + middleware + '.js';
    if ( !pathModule.existsSync(path) ) {
  ...
  // If the `options` param has been omitted.
  if (!property && typeof options == 'string') {
    property = options;
    options = {};
  } else if (!options) {
    options = {};
  ...
  // Middleware can set `this.__noAttach` to true, if it doesn't need
  // its instance to be attached to the application's singleton
  if (!instance.__noAttach) this[property || middleware] = instance;
