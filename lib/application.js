...
var parseRange = framework.util.parseRange;
...
function Application(domain, path) {
  ...
  this.otherMethods = ['OPTIONS', 'PUT', 'DELETE', 'TRACE'];
  ...
  framework.util.requireAllTo(this.mvcpath + "helpers", this.helpers, function(Ctor) {
  ...
  framework.util.requireAllTo(this.mvcpath + "models", this.models, function(Ctor) {
  ...
  this.views.static = framework.util.ls(this.mvcpath + "views/" + this.paths.static, this.regex.templateFile);
...
Application.prototype.httpMethods = ['GET', 'POST', 'PUT', 'DELETE', 'OPTIONS', 'TRACE'];
Application.prototype.httpStatusCodes = http.STATUS_CODES;
...
Application.prototype.use = function(component, options) {
...
Application.prototype.routeRequest = function(req, res) {
  ...
  // Load query data
  if ( req.method == 'GET' && isTypeOf(req.__urlData.query, 'string') ) {
    queryData = qs.parse(req.__urlData.query);
    if (queryData.ajax != null && parseInt(queryData.ajax, 10) === 1) req.__isAjax = true;
...
Application.prototype.login = function(res) {
  ...
  if (controller.className == 'MainController' && route.path === this.loginUrl) {
    route.callback.call(controller, req, res, req.__params);
  } else {
...
Application.prototype.curl = function(cmd, callback) {
  ...
  if (wsIndex >= 0) {
    ...
    cmd = leftStr + ' ' + this.baseUrl + cmd.slice(wsIndex).trim();
  } else {
...
Application.prototype.clientRequest = function(o, callback) {
  ...
  var req = http.request({
    host: self.domain,
    port: framework.config.server.listenPort,
    method: method,
    path: path,
    headers: headers
  }, function(res) {
...
function monkeyPatchController(func) {
  ...
  var alias = framework.lib.controller.prototype.getAlias(func.name),
...
: framework.lib.controller.prototype.authRequired;\n\n\
return " + func.name + ";\n\n\
}\n";
