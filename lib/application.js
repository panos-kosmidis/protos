...
function Application(domain, path) {
  ...
  // Get instances from /mvc/helpers/
  var instance, helperCtor = this.lib.helper || framework.lib.helper;
  framework.util.requireAllTo(this.path + "/mvc/helpers", this.helpers, function(Ctor) {
  ...
  // Get /mvc/models/
  var model, name, modelCtor = this.lib.model || framework.lib.model;
  framework.util.requireAllTo(this.path + "/mvc/models", this.models, function(Ctor) {
  ...
  // Get /mvc/controllers/
  var cpath = path + '/mvc/controllers/',
      files = framework.util.getFiles(cpath);
  
  // Create controllers and attach to app
  var controllerCtor = this.lib.controller || framework.lib.controller;
  ...
  this.views.static = framework.util.ls(this.path + "/mvc/views/" + this.paths.static, this.regex.templateFile);
  ...
  var partialPaths = [];
  fileModule.walkSync(this.path + '/mvc/views', function(dirPath, dirs, files) {
    var layoutPath = self.fullPath('/mvc/views/' + self.paths.layout);
...
Application.prototype.initialize = function() {
  ...
  var intID = setInterval(function() {
    if (self.asyncQueue === 0) {
      // Compensate for non-atomic operations using setTimeout
      setTimeout(function() {
        if (self.asyncQueue === 0) {
          clearInterval(intID); // Clear interval ID (very important!)
          promise.emit('continue'); // Emit continue event for app
          delete self.asyncQueue;
          delete self.asyncQueueTimeout;
        }
      }, self.asyncQueueTimeout);
    }
  }, self.asyncQueueTimeout);
  
  promise.on('continue', function() {
    // Emit the init event
    self.emit('init', self);

    // Run initialization code from init.js
    self.require('init').call(null, self);
...
Application.prototype.use = function(component, options, skipCheck) {
  ...
  if (this.initialized || skipCheck) {
    ...
    if ( pathModule.existsSync(path) ) {
      ...
      path = framework.path + "/lib/addons/" + component + ".js";
      if ( !pathModule.existsSync(path) ) {
...
Application.prototype.buildPartialView = function(path) {
  var pos = path.indexOf('.'),
      ...
      id = func.id = this.relPath(path.substr(0,pos), '/mvc/views')
...
Application.prototype.storage = function(storage, config) {
  return new framework.storages[storage](this, config || {});
}
...
require('util').inherits("+ func.name +", app.lib.controller || framework.lib.controller);\n\n\
...
return " + func.name + ";\n\n\
  ...
  newFunc = compile({
    ...
    require: require,
    console: console,
    __dirname: this.path + '/mvc/controllers',
    __filename: this.path + '/mvc/controllers/' + func.name + '.js',
    process: process
  });
