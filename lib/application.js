...
function Application(domain, path) {
  ...
  corejs.util.requireAllTo(this.mvcpath + "helpers", this.helpers, function(Ctor) {
    ...
    if (Ctor.name == 'MainHelper') corejs.extend(Ctor.prototype, corejs.lib.helper.prototype);
...
Application.prototype.use = function(middleware, options) {
  ...
  if ( pathModule.existsSync(path + '.js') ) {
    ...
    if ( pathModule.existsSync(path + '.js') ) {
      ...
      // Load corejs middleware » middleware.js
      Ctor = require(path + '/' + middleware + '.js');
    } else if ( pathModule.existsSync(path + '.coffee') ) {
      // Load corejs middleware » cs file
      Ctor = require(path + '.coffee');
    } else if ( pathModule.existsSync(path + '/' + middleware + '.coffee') ) {
      // Load corejs middleware » middleware.coffee
      Ctor = require(path + '/' + middleware + '.coffee');
    } else if ( pathModule.existsSync(path) ) {
  ...
  this.debug(util.format("Middleware: %s", middleware));
...
Application.prototype.logFormat = function(local, self, args) {
  ...
  if (typeof msg == 'string' && args.length > 0) { // printf-like args
...
Application.prototype.attachFilter = function(filter, callback) {
...
Application.prototype.applyFilterss = function() {
...
Application.prototype._addClientResource = function(resource, data) {
...
Application.prototype._getClientResource = function(resource, name) {
...
Application.prototype._addClientScript = function(data) {
  this._addClientResource('scripts', data);
}
...
Application.prototype._addClientStylesheet = function(data) {
  this._addClientResource('stylesheets', data);
}
