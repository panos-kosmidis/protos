...
OutgoingMessage.prototype.rawHttpMessage = function(statusCode, message, raw) {
  var buffer, ob, app = this.app;

  switch (typeof statusCode) {
    case 'object':
      // res.rawHttpMessage({statusCode: 404, message: 'not found', raw: true});
      ob = statusCode;
      ...
      break;
    case 'string':
      // res.rawHttpMessage('hello');
      // res.rawHttpMessage('hello', true);
      raw = (typeof message == 'boolean') && message;
      message = statusCode;
      statusCode = 200;
      break;
    case 'number':
      // res.rawHttpMessage(200);
      // res.rawHttpMessage(200, 'hello');
      // res.rawHttpMessage(200, 'hello', true);
...
function renderViewBuffer() {
  ...
  if (runtimeData.views.length === 0) {
    codeBlock = function() {
      ...
      */
      ...
      var data = app.applyFilters(filter, {buffer: runtimeData.buffer, locals: this.viewLocals});
      
      this.sendHeaders();
      this.end(data.buffer, 'utf8');
    }
  ...
  if (viewCaching) {
    if (protos.util.isTypeOf(viewCallbacks[relPath], 'function')) {
      try {
