...
/**
  ...
  Using a path relative to the /app/views/ directory:
    ...
    res.render('/hello') -> will render /hello.{extension} (relative to /app/views)
    res.render('/hello.{extension}') -> will render /hello.{extension} (relative to /app/views)
...
OutgoingMessage.prototype.getViewPath = function(view) {
  ...
  if (app.regex.layoutView.test(view)) {
    ...
    path = app.path + "/app/views/" + app.paths.layout + view;
  } else if (app.regex.restrictedView.test(view)) {
    view = view.replace(app.regex.restrictedView, '');
    path = app.path + "/app/views/" + app.paths.restricted + view;
  } else if (!app.regex.startsWithSlash.test(view) && dirname == '.') {
    alias = controller.getAlias();
    if (app.regex.templateFile.test(view)) {
      path = app.path + "/app/views/" + alias + "/" + view;
    } else {
      path = app.path + "/app/views/" + alias + "/" + alias + "-" + view;
    }
  } else {
    view = view.replace(app.regex.startsWithSlash, '');
    dirname = pathModule.dirname(view);
    if (dirname == '.') {
      path = app.path + "/app/views/" + app.paths.static + view;
    } else {
      depth = (searchPattern(view, '/'))['/'].length;
      if (depth == 1) {
        view = view.split('/');
        path = (app.regex.templateFile.test(view[1]) 
        ? app.path + "/app/views/" + view[0] + "/" + view[1] 
        : app.path + "/app/views/" + view[0] + "/" + view[0] + "-" + view[1]);
      } else {
        path = app.path + "/app/views/" + view;
      }
    }
  }
...
function renderViewBuffer() {
  ...
  relPath = app.relPath(template, 'app/views');
