...
function asyncRender(view, data, raw) {
  ...
  if (!engine) {
    return app.serverError(this, new Error("No engine detected for route: " + request.url));
...
function renderViewBuffer() {
  ...
  if (runtimeData.views.length === 0) {
    codeBlock = function() {
      ...
      var data = app.applyFilters('response_cache', {buffer: runtimeData.buffere, locals: this.viewLocals});
