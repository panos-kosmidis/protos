...
CoreJS.bootstrap = function(dir, config) {
  ...
  appEnv = process.env.NODE_ENV || config.environment.default || 'development';
  ...
  CoreJS.once('pre_init', function(app) {
    ...
    var bootEnvFunc = config.environment[appEnv];
    ...
    if (config.redirect && typeof config.redirect == 'string') {
      app.on('request', function(req, res) {
        ...
        if (req.method == 'GET' || req.method == 'HEAD') res.redirect(config.redirect);
...
function startupMessage(options) {
  ...
  app.log(util.format('[%s] Server running on %s', corejs.environment, app.baseUrl));
